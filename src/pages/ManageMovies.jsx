  ...
  return (
    <Layout pageTitle="Manage Movies" pageDescription="Update, add, or remove titles from your streaming library.">
      ...
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Movie">
        <form className="space-y-6" onSubmit={(e) => handleSubmit(e, 'edit')}>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50 hover:bg-gray-100 cursor-pointer relative overflow-hidden h-40" onClick={() => document.getElementById('file-upload-edit').click()}>
              {uploadingImage ? (
                <img src={uploadingImage} alt="preview" className="w-full h-full object-cover absolute inset-0" />
              ) : (
                <div className="text-center text-gray-400">
